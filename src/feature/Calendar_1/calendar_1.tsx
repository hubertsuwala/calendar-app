...
export const Calendar_1 = () => {
  ...
  const renderEventContent = eventInfo => {
    ...
    return (
      ...
      >
        ...
        <button onClick={buttonClick} className="xxxx xxxxccc">
  ...
  const renderNowIndicatorContent = (eventInfo, isAxis) => {
    ...
    return <div className="indicator">s{/* <button onClick={buttonClick}>{'ggg'}</button> */}</div>;
  };
  return (
    <div className={'calendar-wrapper'}>
      <CalendarHeader
        calendarRef={calendarRef}
        employeeNames={['olo', 'alo']}
        employeeNameHandler={e => {
          console.log(e);
        }}
        setisModalOpen={e => {
          console.log(e);
      ...
      <FullCalendar
        ...
        scrollTime={new Date(Date.now() - 2 * (60 * 60 * 1000)).toTimeString()}
        dayHeaderDidMount={e => {
          console.log(e);
        }}
        eventClassNames={['fffffff', 'fffffffc']}
...
const CalendarHeader = ({ calendarRef, employeeNames, employeeNameHandler, setisModalOpen }) => {
  ...
  return (
    <div className="" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
      <div>
        <button className="" onClick={() => prevHandle()}>
          Prev
        </button>
        <button className="" onClick={() => todayHandle()}>
          Today
        </button>
        <button className="" onClick={() => nextHandle()}>
          Next
        </button>
      </div>

      <div className="">
        <select onChange={employeeNameHandler}>
      ...
      <div>
        <button className="" onClick={() => dayHandle()}>
          Day
        </button>
        <button className="" onClick={() => weekHandle()}>
          Week
        </button>
        {/* <button className="btn-theme text-white f-12 wid-55" onClick={() => monthHandle()}>
          Month
        </button> */}
      </div>
    </div>
